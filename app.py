...
def render_sidebar(config):
    ...
    use_temperature = st.sidebar.toggle(
        "Enviar parámetro temperature",
        value=False,
        help=(
            "Déjalo apagado para máxima compatibilidad. Algunos modelos de razonamiento "
            "solo aceptan la configuración de muestreo por defecto."
        ),
    )
    temperature = None
    if use_temperature:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Generando respuesta con el agente LLM..."):
            try:
                ...
                st.error("Ocurrió un error llamando a OpenAI.")
                st.markdown(
                    "Revisa especialmente: `OPENAI_API_KEY`, el modelo configurado en `OPENAI_MODEL`, "
                    "permisos de la key, saldo/cuota del proyecto y si el modelo admite parámetros como `temperature`."
                )
